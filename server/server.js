...
app.post("/reserve", (req, res) => {
  ...
  const { customerId, name, email, date, time, guests,phoneno, tableNo } = req.body;

  const checkSql = `SELECT id FROM reservations WHERE date=? AND time=? AND table_no=? AND status IN ('Pending','Confirmed')`;

  db.query(checkSql,[ date, time, tableNo],
    ...
    (customer_id, name, email, date, time, guests, phoneno, table_no, status)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'Pending')
  `;

      db.query(
        sql,
        [customerId, name, email, date, time, guests, phoneno, tableNo],
        (err, result) => {
          ...
          if (err) {
            ...
            if (err.code === "ER_DUP_ENTRY") {
              return res.json({
                success: false,
                message: "Someone booked this table just now"
              });
            }
  ...
  app.post("/api/feedback", (req, res) => {
    const { name, email, phoneno, rating, message } = req.body;

    const sql =
      "INSERT INTO feedback (name, email, phone_no, rating, message) VALUES (?, ?, ?, ?, ?)";

    db.query(sql, [name, email, phoneno, rating, message], (err, result) => {
      if (err) return res.status(500).send(err);
